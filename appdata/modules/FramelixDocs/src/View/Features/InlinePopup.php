...
class InlinePopup extends View
{
    ...
    public function showContent(): void
    {
        ?>
        <p>
            With our Javascript class <code>FramelixPopup</code> you can create containers that are attached to a element.
            It will stay there, flips automatically when scrolling comes to the edges of the popup, and have other cool features. The most obvious example of this is <code title="Yep, that's an inline popup">"Tooltip" (Hover this to see what we mean)</code>.
            <br/>
        ...
        ?>
        <p>
            There are many options for the <code>FramelixPopup.show</code> call. To modify, size, width, behaviour, style, contents, listen for events and many more. Here are the corresponding doc            comments for that.
        </p>
        <?php
        $this->addSourceFile(FRAMELIX_APPDATA_FOLDER."/modules/Framelix/js/framelix-popup.js", 'FramelixPopupShowOptions');
        $this->showSourceFiles();
        ?>
        <p>
            The complete source including other functions can be found at <?=$this->getShowSourceFileLinkTag(['Framelix/js/framelix-popup.js'])?>
        </p>
        <?php
    }
}